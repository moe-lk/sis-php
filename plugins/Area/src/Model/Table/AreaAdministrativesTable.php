...
class AreaAdministrativesTable extends ControllerActionTable
{
            ...
            ->add('is_main_country', 'ruleValidateAreaAdministrativeMainCountry', [
                ...
                'on' => function ($context) {
                    ...
                    $worldQuery = $this
                        ...
                        ->where([$this->aliasField('code') => 'World'])
                        ->first();
                    $query = $this->find()
                            ->select([$this->aliasField('id')])
                            ->where([$this->aliasField('parent_id') => $worldQuery['id']])
                            ->first();
    ...
    public function onUpdateFieldIsMainCountry(Event $event, array $attr, $action, Request $request)
    {
        if ($action == 'add') {
            ...
            $countryLevel = $this->AreaAdministrativeLevels->findByName('Country')->first()->id;
            if ($areaAdminLevelId == $countryLevel) {
