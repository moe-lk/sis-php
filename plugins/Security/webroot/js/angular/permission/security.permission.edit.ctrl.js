...
function SecurityPermissionEditController($scope, $q, $window, $http, UtilsSvc, AlertSvc, SecurityPermissionEditSvc) {
    ...
    angular.element(document).ready(function () {
        ...
        Controller.ready = false;
        console.log(Controller.includedModules);
        var modules = JSON.parse(Controller.includedModules);
        for (var i = 0; i < modules.length; i++) {
            console.log(modules[i]);
            Controller.modules.push({'key': modules[i], 'name': modules[i]});
        }
