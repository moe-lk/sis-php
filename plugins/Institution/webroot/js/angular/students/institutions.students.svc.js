...
InstitutionsStudentsSvc.$inject = [
  "$http",
  "$q",
  "$filter",
  "KdOrmSvc",
...
function InstitutionsStudentsSvc($http, $q, $filter, KdOrmSvc, KdDataSvc) {
  ...
  function getAdminitrativeAreas(
    model,
    userId,
    displayCountry,
    selected,
  ...
  function getExternalSourceAttributes() {
    return ExternalDataSourceAttributes.find("attributes").ajax({
      defer: true,
    });
  }

  function getExternalSourceUrl() {
    return ExternalDataSourceAttributes.find("Uri", {
    ...
    vm.getExternalSourceAttributes()
      .then(
        function (attributes) {
          ...
          var url =
            angular.baseUrl +
            "/Configurations/getExternalUsers?page={page}&limit={limit}&first_name={first_name}&last_name={last_name}&identity_number={identity_number}&date_of_birth={date_of_birth}";
          var pageParams = {
            ...
            for (var key in options["conditions"]) {
              if (typeof options["conditions"][key] == "string") {
                ...
                if (key != "date_of_birth") {
                  ...
                  params[key] = vm.formatDateForSaving(
      ...
      for (var key in options["conditions"]) {
        if (typeof options["conditions"][key] == "string") {
          ...
          if (options["conditions"][key] !== "") {
            if (key == "date_of_birth") {
              options["conditions"][key] = vm.formatDate(
                options["conditions"][key]
              );
            }
            params[key] = options["conditions"][key];
  ...
  function getStudentData(id) {
    ...
    return StudentUser.contain(["Genders", "Identities.IdentityTypes"])
  ...
  function addIdentityType(identityType) {
    ...
    IdentityTypes.save(identityType).then(
      function (response) {
  ...
  function addUser(userRecord) {
    ...
    if (externalSource == null) {
      userRecord["start_date"] = vm.formatDateForSaving(
        userRecord["start_date"]
      );
      userRecord["institution_id"] = vm.getInstitutionId();
      delete userRecord["gender"];
      if (
        userRecord["nationality_id"] != "" &&
        userRecord["nationality_id"] != undefined
      ) {
      ...
      if (
        userRecord["identity_type_id"] != "" &&
        userRecord["identity_type_id"] != undefined &&
        userRecord["identity_number"] != undefined &&
        userRecord["identity_number"] != ""
      ) {
      ...
      StudentUser.reset();
      StudentUser.save(userRecord).then(
      ...
      var genderName = "";
      var identityNumber = "";
      vm.getExternalSourceAttributes().then(
        function (attributes) {
          var attr = attributes.data;

          // Mandatory information from the form
          newUserRecord["academic_period_id"] =
            userRecord["academic_period_id"];
          newUserRecord["education_grade_id"] =
            userRecord["education_grade_id"];
          newUserRecord["start_date"] = vm.formatDateForSaving(
            userRecord["start_date"]
          );
          ...
          newUserRecord["preferred_name"] =
            userRecord[attr["preferred_name_mapping"]];
          newUserRecord["date_of_birth"] =
            userRecord[attr["date_of_birth_mapping"]];
          newUserRecord["external_reference"] =
            userRecord[attr["external_reference_mapping"]];
          ...
          if (typeof userRecord[attr["middle_name_mapping"]] != "undefined") {
            newUserRecord["middle_name"] =
              userRecord[attr["middle_name_mapping"]];
          }
          if (typeof userRecord[attr["third_name_mapping"]] != "undefined") {
            newUserRecord["third_name"] =
              userRecord[attr["third_name_mapping"]];
          }
          if (
            typeof userRecord[attr["identity_number_mapping"]] != "undefined"
          ) {
          ...
          vm.getUserRecord(newUserRecord["external_reference"]).then(
            function (response) {
              if (response.data.length > 0) {
                ...
                modifiedUser["academic_period_id"] =
                  userRecord["academic_period_id"];
                modifiedUser["education_grade_id"] =
                  userRecord["education_grade_id"];
                modifiedUser["start_date"] = vm.formatDateForSaving(
                  userRecord["start_date"]
                );
                StudentUser.edit(modifiedUser).then(
                  function (response) {
                ...
                newUserRecord["date_of_birth"] = vm.formatDateForSaving(
                  newUserRecord["date_of_birth"]
                );
                newUserRecord["is_student"] = 1;

                vm.importMappingObj(genderName, nationality, identityType).then(
                  function (promiseArr) {
                    ...
                    newUserRecord["username"] = newUserRecord[
                      "openemis_no"
                    ].replace("-", "");
                    delete newUserRecord["password"];
                    var identityTypeId = promiseArr[2];
                    StudentUser.reset();
                    StudentUser.save(newUserRecord).then(
                      function (studentRecord) {
  ...
  function addUserNationality(userId, nationalityId) {
    ...
    return UserNationalities.save(data);
  }
  ...
  function getGenders() {
    ...
    return Genders.select().ajax({ success: success, defer: true });
  }
  ...
  function getEducationGrades(options) {
    ...
    if (
      ...
      options.hasOwnProperty("academicPeriodId")
    ) {
  ...
  function getClasses(options) {
    ...
    return InstitutionClasses.select()
      .find("ClassOptions", {
        institution_id: options.institutionId,
        academic_period_id: options.academicPeriodId,
  ...
  function getAddNewStudentConfig() {
    return ConfigItems.select()
      .where({ type: "Add New Student" })
      .ajax({ defer: true });
  }

  function getUserContactTypes() {
    return ContactTypes.select().ajax({ defer: true });
  }
  ...
  function getNationalities() {
    return Nationalities.select()
      .contain(["IdentityTypes"])
      .ajax({ defer: true });
  }

  function getSpecialNeedTypes() {
    return SpecialNeedTypes.select().ajax({ defer: true });
  }

  function getStudentTransferReasons() {
    return StudentTransferReasons.select().ajax({ defer: true });
  }
}
