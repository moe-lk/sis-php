...
angular
  .module("institution.class.students.ctrl", [
    "agGrid",
  ...
  .controller(
    "InstitutionClassStudentsCtrl",
    InstitutionClassStudentsController
  );

InstitutionClassStudentsController.$inject = [
  "$scope",
  "$q",
  "$window",
  "$http",
...
function InstitutionClassStudentsController(
  $scope,
  $q,
  $window,
  $http,
      ...
      InstitutionClassStudentsSvc.getClassDetails(Controller.classId)
        .then(
          function (response) {
            ...
            Controller.noStudents = response.no_of_students;
            ...
            var promises = [];
            promises[0] = InstitutionClassStudentsSvc.getUnassignedStudent(
              Controller.classId
            );
            promises[1] = InstitutionClassStudentsSvc.getInstitutionShifts(
              response.institution_id,
              response.academic_period_id
            );
            promises[2] = InstitutionClassStudentsSvc.getTeacherOptions(
              response.institution_id,
              response.academic_period_id
            );
            promises[3] = InstitutionClassStudentsSvc.getConfigItemValue(
  ...
  function postForm() {
    ...
    postData.no_of_students = Controller.noStudents;
    ...
    if (classStudents.length > Controller.noStudents) {
      AlertSvc.error(
        Controller,
        "The number of students per class has reached the  limit of " +
          Controller.noStudents +
          " students."
      );
    } else if (classStudents.length > Controller.maxStudentsPerClass) {
      AlertSvc.error(
        Controller,
        "The number of students per class has reached the maximum limit of " +
          Controller.maxStudentsPerClass +
          " students."
      );
    } else {
      InstitutionClassStudentsSvc.saveClass(postData).then(
        function (response) {
