...
class POCOR4499 extends AbstractMigration
{
    public function up()
    {
        ...
        // security_functions
        $select = '
        SELECT `order` FROM `security_functions` WHERE `id` = 3041
        ';
        $row = $this->fetchRow($select);
        $order = $row['order'];

        $update = '
        UPDATE `security_functions` SET `order` = `order` + 1 WHERE `order` >= ' . $order;
        $this->execute($update);
        ...
        $select2 = '
        SELECT `order` FROM `security_functions` WHERE `id` = 7055';
        $row = $this->fetchRow($select2);
        $order = $row['order'];
